#!/usr/bin/env bash
# Sweep Hidden-P projection mismatch. The rotated modes are the main robustness
# study; the independent mode is an extreme stress test with random P_M.
set -euo pipefail
...
for angle in 0 5 15 30; do
  out_dir="${OUT_ROOT}/rotated_angle_${angle}"
...
python "${SCRIPT_DIR}/synthetic_validate_theory.py" \
  ...
  --pm-mode independent \
  --pm-angle-deg 0 \
  --output-dir "${OUT_ROOT}/independent"

python "${SCRIPT_DIR}/plot_hidden_p_sweep.py" \
  --results-root "${OUT_ROOT}" \
  --noise-std 0.5

echo "Done. Sweep results in ${OUT_ROOT}"
