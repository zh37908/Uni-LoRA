...
class TestSameAdapterDifferentDevices:
    ...
    def test_unilora_add_new_adapter_does_not_change_device(self, mlp):
        # same as first test, but using VBLoRA
        config = UniLoRAConfig(target_modules=["lin0"], vector_length=2)
        model = get_peft_model(mlp, config)
        model = model.cuda()
        model.lin0.unilora_logits_A.cpu()
        model.lin0.unilora_logits_B.cpu()
        model.lin0.unilora_vector_bank.cpu()

        # check that the adapter is indeed on CPU and the base model on GPU
        assert model.lin0.unilora_logits_A.default.device.type == "cpu"
        assert model.lin0.unilora_logits_B.default.device.type == "cpu"
        assert model.lin0.unilora_vector_bank.default.device.type == "cpu"
        assert model.lin0.base_layer.weight.device.type == "cuda"
        ...
        # the rest should be on GPU
        assert model.lin0.base_layer.weight.device.type == "cuda"
        assert model.lin0.unilora_logits_A.other.device.type == "cuda"
        assert model.lin0.unilora_logits_B.other.device.type == "cuda"
        assert model.lin0.unilora_vector_bank.other.device.type == "cuda"
