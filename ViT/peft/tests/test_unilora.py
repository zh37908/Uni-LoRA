...
class TestVBLoRA:
    ...
    def test_unilora_parameters(self):
        ...
        vector_bank = mlp_unilora.unilora_vector_bank["default"]

        unilora_lin0_logits_B = mlp_unilora.lin0.unilora_logits_B["default"]
        assert unilora_lin0_logits_B.shape == (mlp.lin0.out_features // vector_length, config.r, num_vectors)

        unilora_lin1_logits_A = mlp_unilora.lin1.unilora_logits_A["default"]
        assert unilora_lin1_logits_A.shape == (config.r, mlp.lin1.in_features // vector_length, num_vectors)

        unilora_lin3_logits_A = mlp_unilora.lin3.unilora_logits_A["default"]
        assert unilora_lin3_logits_A.shape == (config.r, mlp.lin3.in_features // vector_length, num_vectors)

        assert vector_bank.shape == (num_vectors, vector_length)

        # test if the vector bank is shared across the layers
        assert (
            mlp_unilora.lin0.unilora_vector_bank["default"].data_ptr()
            == mlp_unilora.lin3.unilora_vector_bank["default"].data_ptr()
        )
        assert mlp_unilora.lin1.unilora_vector_bank["default"].data_ptr() == vector_bank.data_ptr()
    ...
    def test_save_with_topk_weights(self, tmp_path):
        ...
        assert "base_model.model.lin0.unilora_logits_A_topk_indices" in adapter_model_dict
        assert "base_model.model.lin0.unilora_logits_A_topk_weights" in adapter_model_dict
        assert "base_model.model.lin3.unilora_logits_B_topk_indices" in adapter_model_dict
        assert "base_model.model.lin3.unilora_logits_B_topk_weights" in adapter_model_dict
        assert "base_model.model.lin0.unilora_logits_A" not in adapter_model_dict
        assert "base_model.model.lin3.unilora_logits_B" not in adapter_model_dict

        assert adapter_model_dict["base_model.model.lin0.unilora_logits_B_topk_indices"].shape == (
            mlp.lin0.out_features // vector_length,
            config.r,
            topk,
        )
        assert adapter_model_dict["base_model.model.lin0.unilora_logits_B_topk_weights"].shape == (
            mlp.lin0.out_features // vector_length,
            config.r,
            topk - 1,
        )
        assert adapter_model_dict["base_model.model.lin3.unilora_logits_A_topk_indices"].shape == (
            config.r,
            mlp.lin3.in_features // vector_length,
            topk,
        )
        assert adapter_model_dict["base_model.model.lin3.unilora_logits_A_topk_weights"].shape == (
            config.r,
            mlp.lin3.in_features // vector_length,
            topk - 1,
        )
    ...
    def test_resume_training_model_with_topk_weights(self, tmp_path):
        ...
        mlp = self.get_mlp()
        mlp_unilora_loaded = PeftModel.from_pretrained(mlp, save_path)
        mlp_unilora_loaded.train()
        msg = "Found infinity values in Uni-LoRA logits. Ensure training was not resumed from a `save_only_topk_weights` model."
        with pytest.raises(RuntimeError, match=msg):
            mlp_unilora_loaded(input)
    ...
    def test_unilora_dtypes(self, dtype):
        ...
        output = mlp_unilora(inputs)  # should not raise
        assert output.dtype == dtype

    def test_unilora_nb_savable_params_only_topk_weights(self):
        mlp = self.get_mlp()
        vector_length = 2
        num_vectors = 10
        topk = 2
        ...
        mlp_unilora.lin3.requires_grad_(True)  # set lin3 to trainable
    ...
    def test_unilora_nb_savable_params_all_logits(self):
        mlp = self.get_mlp()
        vector_length = 2
        num_vectors = 10
        topk = 2
