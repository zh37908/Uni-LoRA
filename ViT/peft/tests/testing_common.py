...
CONFIG_TESTING_KWARGS = (
    ...
    # VBLoRA
    {"target_modules": None, "unilora_dropout": 0.05, "vector_length": 1, "num_vectors": 2},
)
...
CLASSES_MAPPING = {
    ...
    "unilora": (UniLoRAConfig, CONFIG_TESTING_KWARGS[10]),
}
...
class PeftCommonTester:
    ...
    def _test_merge_layers_fp16(self, model_id, config_cls, config_kwargs):
        if config_cls not in (LoraConfig, IA3Config, AdaLoraConfig, LoHaConfig, LoKrConfig, UniLoRAConfig):
    ...
    def _test_peft_model_device_map(self, model_id, config_cls, config_kwargs):
        if config_cls not in (LoraConfig, UniLoRAConfig):
            return pytest.skip(f"Test not applicable for {config_cls}")
    ...
    def _test_unload_adapter(self, model_id, config_cls, config_kwargs):
        ...
        if config.peft_type not in ("LORA", "ADALORA", "IA3", "BOFT", "VERA", "FOURIERFT", "HRA", "UNILORA"):
