...
TEST_CASES = [
    ...
    # VBLoRA #
    ##########
    ("Vanilla MLP 1 VBLoRA", "MLP", UniLoRAConfig, {"target_modules": "lin0", "vector_length": 1, "num_vectors": 5}),
    ("Vanilla MLP 2 VBLoRA", "MLP", UniLoRAConfig, {"target_modules": ["lin0"], "vector_length": 1, "num_vectors": 5}),
    ("Vanilla MLP 3 VBLoRA", "MLP", UniLoRAConfig, {"target_modules": ["lin1"], "vector_length": 2, "num_vectors": 5}),
    (
...
class PeftCustomModelTester(unittest.TestCase, PeftCommonTester):
    ...
    def test_only_params_are_updated(self, test_name, model_id, config_cls, config_kwargs):
        ...
        lr = (
            100.0
            if (config_kwargs.get("use_dora") and model_id == "EmbConv1D") or issubclass(config_cls, UniLoRAConfig)
            else 0.5
        )
    ...
    def test_disable_adapters(self, test_name, model_id, config_cls, config_kwargs):
        ...
        model = get_peft_model(model, config)
        if issubclass(config_cls, UniLoRAConfig):
            # Manually set the `unilora_vector_bank` to zero so that Uni-LoRA functions as an identity operation.
            torch.nn.init.zeros_(model.unilora_vector_bank["default"])
        model.eval()
        outputs_before = model(**X)
        assert torch.allclose(outputs_base, outputs_before)

        if issubclass(config_cls, UniLoRAConfig):
            # initialize `unilora_vector_bank` so it can be trained
            model._init_unilora_vector_bank(config, "default")
        model.train()
...
class RequiresGradTester(unittest.TestCase):
    ...
    def test_requires_grad_unilora_different_targets(self):
        # test two different VBLoRA adapters that target different modules
        config0 = UniLoRAConfig(target_modules=["lin0"], vector_length=1, num_vectors=2)
        peft_model = get_peft_model(MLP(), config0)

        config1 = UniLoRAConfig(target_modules=["lin1"], vector_length=1, num_vectors=2)
        peft_model.add_adapter("adapter1", config1)

        # active adapter is still "default"
        self.check_requires_grad(
            peft_model,
            "base_model.model.lin0.unilora_logits_A.default",
            "base_model.model.lin0.unilora_logits_B.default",
            "base_model.model.lin0.unilora_vector_bank.default",
        )
        ...
        self.check_requires_grad(
            peft_model,
            "base_model.model.lin1.unilora_logits_A.adapter1",
            "base_model.model.lin1.unilora_logits_B.adapter1",
            "base_model.model.lin0.unilora_vector_bank.adapter1",  # unilora_vector_bank is shared
        )
    ...
    def test_requires_grad_unilora_same_targets(self):
        # same as previous test, except that VBLoRA adapters target the same layer
        config0 = UniLoRAConfig(target_modules=["lin0"], vector_length=1, num_vectors=2)
        peft_model = get_peft_model(MLP(), config0)

        config1 = UniLoRAConfig(target_modules=["lin0"], vector_length=1, num_vectors=2)
        peft_model.add_adapter("adapter1", config1)
        ...
        self.check_requires_grad(
            peft_model,
            "base_model.model.lin0.unilora_logits_A.adapter1",
            "base_model.model.lin0.unilora_logits_B.adapter1",
            "base_model.model.lin0.unilora_vector_bank.adapter1",
        )
