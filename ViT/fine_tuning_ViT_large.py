...
parser.add_argument("--dataset", type=str, default="fgvc", choices=["oxfordpets","fgvc","standfordcars","cifar10", "cifar100", "dtd","flowers102","eurosat", "resisc45"])
...
if dataset_name == "cifar10":
    ...
    val_ds = train_valid_split['test']
    train_ds = train_valid_split['train']
    test_ds = load_dataset("tanganke/stanford_cars")
elif dataset_name == "fgvc": 
    train_ds = load_dataset("fgvc-aircraft/fgvc-aircraft.py", split="train", trust_remote_code=True) 
    val_ds = load_dataset("fgvc-aircraft/fgvc-aircraft.py", split="validation", trust_remote_code=True) 
    test_ds = load_dataset("fgvc-aircraft/fgvc-aircraft.py", split="test", trust_remote_code=True) 

elif dataset_name == "dtd": 
    ds = load_dataset("imagefolder", data_dir="Describable-Textures-Dataset-DTD", split="train")

    # 
    ds = ds.shuffle(seed=42)
    train_val_test = ds.train_test_split(test_size=0.28, seed=42)
    val_test = train_val_test['test'].train_test_split(test_size=0.715, seed=42)

    train_ds = train_val_test['train']         # ~4060
    val_ds   = val_test['train']               # ~452
    test_ds  = val_test['test']                # ~1128

    # 
    print(len(train_ds), len(val_ds), len(test_ds))  # ：4060, 452, 1128
