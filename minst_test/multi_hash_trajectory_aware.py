...
def build_results_path(args):
    ...
    return "mnist_trajectory_aware_br{}_bc{}_compress{}_seed{}.json".format(
        args.block_rows,
        args.block_cols,
        args.compress,
        args.seed,
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="PyTorch Trajectory-Aware HashNet on MNIST",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Multiply current learning rate by this factor after each accepted E-step",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save-model-path",
        type=str,
        default="mnist_trajectory_aware.pt",
...
def compute_reassignment_gains(
    weight_blocks,
    predicted_blocks,
    old_assignments,
    new_assignments,
    ...
    weight_blocks = weight_blocks.to(work_device)
    predicted_blocks = predicted_blocks.to(work_device)
    ...
    num_clusters = int(max(old_assignments.max().item(), new_assignments.max().item()) + 1)
    old_weight_centers, _ = compute_cluster_centers(weight_blocks, old_assignments, num_clusters)
    new_weight_centers, _ = compute_cluster_centers(weight_blocks, new_assignments, num_clusters)
    old_pred_centers, _ = compute_cluster_centers(predicted_blocks, old_assignments, num_clusters)
    new_pred_centers, _ = compute_cluster_centers(predicted_blocks, new_assignments, num_clusters)

    gains = torch.empty(weight_blocks.size(0), device=work_device, dtype=weight_blocks.dtype)
    weight_gains = torch.empty_like(gains)
    pred_gains = torch.empty_like(gains)

    for start in range(0, weight_blocks.size(0), chunk_size):
        weight_chunk = weight_blocks[start:start + chunk_size]
        pred_chunk = predicted_blocks[start:start + chunk_size]
        old_ids = old_assignments[start:start + chunk_size]
        new_ids = new_assignments[start:start + chunk_size]

        old_weight_err = (weight_chunk - old_weight_centers[old_ids]).square().mean(dim=1)
        new_weight_err = (weight_chunk - new_weight_centers[new_ids]).square().mean(dim=1)
        old_pred_err = (pred_chunk - old_pred_centers[old_ids]).square().mean(dim=1)
        new_pred_err = (pred_chunk - new_pred_centers[new_ids]).square().mean(dim=1)

        weight_gain_chunk = weight_scale * (old_weight_err - new_weight_err)
        pred_gain_chunk = pred_scale * (old_pred_err - new_pred_err)
        weight_gains[start:start + chunk_size] = weight_gain_chunk
        pred_gains[start:start + chunk_size] = pred_gain_chunk
        gains[start:start + chunk_size] = weight_gain_chunk + pred_gain_chunk

    return gains.cpu(), weight_gains.cpu(), pred_gains.cpu()
...
class BlockTrajectoryCollector:
    ...
    def get_block_targets(self):
        if self.num_updates == 0:
            raise ValueError("No trajectory signatures were collected")

        denom = float(self.num_updates)
        return {
            "weight_blocks": self.sum_weight_blocks / denom,
            "grad_blocks": self.sum_grad_blocks / denom,
            "predicted_blocks": self.sum_pred_blocks / denom,
        }
...
def recluster_clustered_model(clustered_model, collectors, args, event_index):
    ...
    for index, ((layer_name, clustered_layer), collector) in enumerate(zip(clustered_layers, collectors)):
        ...
        features = collector.get_features()
        block_targets = collector.get_block_targets()
        old_assignments = clustered_layer.block_assignments.detach().cpu()
        new_assignments, _ = run_kmeans(
            features,
        ...
        gains, weight_gains, pred_gains = compute_reassignment_gains(
            block_targets["weight_blocks"],
            block_targets["predicted_blocks"],
            old_assignments,
            new_assignments,
            weight_scale=max(args.weight_feature_scale, 1e-8),
            pred_scale=max(args.pred_feature_scale, 1e-8),
            device=gain_device,
        )
        ...
        info["mean_weight_gain_all_changed"] = float(weight_gains[candidate_indices].mean().item()) if num_candidates > 0 else 0.0
        info["mean_pred_gain_all_changed"] = float(pred_gains[candidate_indices].mean().item()) if num_candidates > 0 else 0.0
        info["mean_gain_applied"] = float(gains[selected_indices].mean().item()) if selected_indices.numel() > 0 else 0.0
        info["mean_weight_gain_applied"] = float(weight_gains[selected_indices].mean().item()) if selected_indices.numel() > 0 else 0.0
        info["mean_pred_gain_applied"] = float(pred_gains[selected_indices].mean().item()) if selected_indices.numel() > 0 else 0.0
        info["gain_metric"] = "current_weight_error_plus_one_step_prediction_error"
        info["gain_weight_scale"] = float(max(args.weight_feature_scale, 1e-8))
        info["gain_pred_scale"] = float(max(args.pred_feature_scale, 1e-8))
...
def main():
    ...
    print("Trajectory-aware parameter count: {}".format(parameter_count))

    finetune_epochs = args.epochs - args.warmup_epochs
    em_collectors = make_block_collectors(clustered_model, args) if args.em_interval > 0 else None
    em_event_index = 0
    for offset in range(1, finetune_epochs + 1):
        ...
        if args.em_interval > 0 and offset % args.em_interval == 0 and offset < finetune_epochs:
            ...
            em_collectors = make_block_collectors(clustered_model, args)
