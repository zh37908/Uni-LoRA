...
@dataclass
class UniLoRAMultiStructuredGlobalConfig(PeftConfig):
    """
    UniLoRA multi-structured-global variant:
    conceptually tile all LoRA A/B elements into one global matrix and parameterize that matrix with SMH.
    """

    r: int = field(default=4, metadata={"help": "The rank of incremental matrices."})
    proj_seed: int = field(default=42, metadata={"help": "Reserved for deterministic layout extensions."})
    ...
    num_hash_pairs: int = field(
        default=4,
        metadata={"help": "Number of sum-of-products pairs M used to reconstruct each global matrix value."},
    )
    ...
    init_bound: float = field(
        default=0.02,
        metadata={"help": "Uniform init bound for shared global SMH factor banks."},
    )
    ...
    def __post_init__(self):
        self.peft_type = PeftType.UNILORA_MULTI_STRUCTURED_GLOBAL
