...
class UniLoRAAromaLayer(BaseTunerLayer):
    adapter_layer_names = ("unilora_aroma_theta_d",)
    ...
    def __init__(self, base_layer: nn.Module, **kwargs):
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            raise ValueError(f"Unsupported base layer type {type(base_layer)} for UniLoRA-AROMA.")
    ...
    def update_layer(
        ...
        self.unilora_aroma_theta_d = unilora_aroma_theta_d
    ...
    def reset_unilora_parameters(self, adapter_name: str, theta_d_length: int):
        if adapter_name in self.unilora_aroma_theta_d.keys():
    ...
    def update_norm(self, adapter_name: str, unilora_scales_A: torch.Tensor, unilora_scales_B: torch.Tensor):
        if adapter_name not in self.unilora_aroma_theta_d.keys():
            return
    ...
    def set_structure(
        ...
        target_device = base_layer.weight.device
        target_dtype = base_layer.weight.dtype
        self.unilora_indices_A[adapter_name] = indices_A.to(device=target_device, dtype=torch.long)
        self.unilora_indices_B[adapter_name] = indices_B.to(device=target_device, dtype=torch.long)
        self.unilora_scales_A[adapter_name] = scales_A.to(device=target_device, dtype=target_dtype)
        self.unilora_scales_B[adapter_name] = scales_B.to(device=target_device, dtype=target_dtype)


class Linear(nn.Linear, UniLoRAAromaLayer):
    def __init__(
        self,
        base_layer,
        unilora_aroma_theta_d,
        ...
        **kwargs,
        ...
        UniLoRAAromaLayer.__init__(self, base_layer, **kwargs)
        self.fan_in_fan_out = fan_in_fan_out
        self._active_adapter = adapter_name
        self.update_layer(adapter_name, unilora_aroma_theta_d, r, theta_d_length, unilora_dropout)
    ...
    def _get_lora_matrices(self, adapter: str, cast_to_fp32: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        theta_d = self.unilora_aroma_theta_d[adapter].to(indices_A.device)
    ...
    def get_delta_weight(self, adapter: str) -> torch.Tensor:
        ...
        dtype = self.unilora_aroma_theta_d[adapter].dtype
        cast_to_fp32 = device.type == "cpu" and dtype == torch.float16
        A, B = self._get_lora_matrices(adapter, cast_to_fp32=cast_to_fp32)
        return transpose(B @ A, self.fan_in_fan_out)

    @torch.no_grad()
    def merge_current_adapter_into_base(self, adapter: str) -> torch.Tensor:
        delta = self.get_delta_weight(adapter)
        self.get_base_layer().weight.data += delta.to(dtype=self.get_base_layer().weight.dtype)
        return delta
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                x_cast = x.to(self.unilora_aroma_theta_d[active_adapter].dtype)
