...
@dataclass
class UniLoRASketchDeltaConfig(PeftConfig):
    """
    UniLoRA variant where the LoRA delta matrices A/B are reconstructed from
    per-layer sketch codebooks with fixed discrete assignments.
    """

    r: int = field(default=4, metadata={"help": "The rank of incremental matrices."})
    proj_seed: int = field(default=42, metadata={"help": "Random seed for sketch code initialization."})
    bits: int = field(default=4, metadata={"help": "Codebook bit-width for the sketch-parameterized delta matrices."})
    ...
    unilora_dropout: float = field(default=0.0, metadata={"help": "Dropout applied before the sketch delta branch."})
    ...
    init_codebook_bound: float = field(
        default=0.02,
        metadata={"help": "Initialize sketch codebooks with U(-bound, bound)."},
    )
    ...
    def __post_init__(self):
        self.peft_type = PeftType.UNILORA_SKETCH_DELTA
