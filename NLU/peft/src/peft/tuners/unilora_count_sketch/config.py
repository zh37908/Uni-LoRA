...
@dataclass
class UniLoRACountSketchConfig(PeftConfig):
    r: int = field(default=4, metadata={"help": "The rank of incremental matrices."})
    proj_seed: int = field(default=42, metadata={"help": "Random seed for projection initialization."})
    theta_d_length: int = field(default=256, metadata={"help": "The length of each theta_d vector."})
    num_sketches: int = field(default=3, metadata={"help": "Number of independent Count-Sketch projections (v)."})
    ...
    init_theta_d_bound: float = field(default=0.02, metadata={"help": "Initialize theta_d with U(-b, b)."})
    ...
    def __post_init__(self):
        if self.num_sketches <= 0:
            raise ValueError(f"`num_sketches` must be > 0, got {self.num_sketches}.")
        self.peft_type = PeftType.UNILORA_COUNT_SKETCH
