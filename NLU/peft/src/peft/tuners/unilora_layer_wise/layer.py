...
class UniLoRALayerWiseLayer(BaseTunerLayer):
    """
    UniLoRA Layer-Wise variant: each layer has an INDEPENDENT theta_d.
    """

    adapter_layer_names = ("theta_d",)

    def __init__(self, base_layer: nn.Module, **kwargs):
        self.base_layer = base_layer
        self.r = {}
        self.unilora_dropout = nn.ModuleDict({})
        # Note: No global scaling dict needed as layers are independent

        # FastFood buffers (per-adapter)
        ...
        # Local parameters (per-adapter)
        self.theta_d = nn.ParameterDict({})
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            raise ValueError(f"Unsupported base layer type {type(base_layer)} for UniLoRALayerWiseLayer.")
    ...
    def update_layer(
        ...
        if theta_d_length_local <= 0:
            raise ValueError(f"`theta_d_length_local` {theta_d_length_local} should be a positive integer value")
    ...
    def _reset_fastfood_buffers(self, adapter_name: str, proj_seed: int, layer_seed: int):
        if adapter_name not in self.theta_d.keys():
            return
...
class Linear(nn.Linear, UniLoRALayerWiseLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(
            ...
            r=r,
            theta_d_length_local=theta_d_length_local,
            proj_seed=proj_seed,
    ...
    def _get_lora_matrices(self, adapter, cast_to_fp32=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # No global scaling since layers are independent
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        dtype = self.theta_d[adapter].dtype
