...
@dataclass
class UniLoRARoSAStageConfig(PeftConfig):
    """
    UniLoRA with a stage-wise RoSA sparse compensation vector.

    The sparse adapter is activated after a configurable fraction of the full
    training epochs has elapsed. At that stage, gradients are collected for a
    small number of optimizer steps to determine sparse positions, and then the
    sparse parameters are trained with the selected mask fixed.
    """
    ...
    target_modules: Optional[Union[List[str], str]] = field(
        ...
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with UniLoRA-RoSA-Stage. "
    ...
    bias: str = field(default="none", metadata={"help": "Bias type for UniLoRA-RoSA-Stage adapters."})
    ...
    rosa_density: float = field(
        default=0.01,
        metadata={"help": "Target density for the sparse compensation vector S."},
    )
    rosa_stage_ratio: float = field(
        default=0.5,
        metadata={
            "help": (
                "Fraction of the full training epochs completed before starting RoSA sparse-mask collection. "
                "For example, 0.5 activates sparse structure after 50% of training."
            )
        },
    )
    ...
    def __post_init__(self):
        self.peft_type = PeftType.UNILORA_ROSA_STAGE
        self.target_modules = set(self.target_modules) if isinstance(self.target_modules, list) else self.target_modules

        if self.r <= 0:
            raise ValueError("`r` must be a positive integer for UniLoRA-RoSA-Stage.")
        if self.theta_d_length <= 0:
            raise ValueError("`theta_d_length` must be positive for UniLoRA-RoSA-Stage.")
        if not 0.0 <= self.rosa_density <= 1.0:
            raise ValueError("`rosa_density` must be in [0, 1].")
        if not 0.0 <= self.rosa_stage_ratio <= 1.0:
            raise ValueError("`rosa_stage_ratio` must be in [0, 1].")
