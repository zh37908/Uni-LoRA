...
class UniLoRAMultiHashingLayer(BaseTunerLayer):
    adapter_layer_names = ("unilora_multi_hashing_theta_d", "unilora_multi_hashing_P")
    ...
    def update_layer(
        ...
        self.unilora_multi_hashing_theta_d = unilora_multi_hashing_theta_d
        self.unilora_multi_hashing_P = unilora_multi_hashing_P
    ...
    def reset_unilora_parameters(self, adapter_name: str, theta_d_length: int):
        if adapter_name not in self.unilora_multi_hashing_theta_d:
            return

        self.unilora_indices_A[adapter_name] = torch.randint(
            0, theta_d_length, (self.r[adapter_name], self.in_features), dtype=torch.long
        )
        self.unilora_indices_B[adapter_name] = torch.randint(
...
class Linear(nn.Linear, UniLoRAMultiHashingLayer):
    def __init__(
        self,
        base_layer,
        unilora_multi_hashing_theta_d,
        ...
        **kwargs,
        ...
        UniLoRAMultiHashingLayer.__init__(self, base_layer, **kwargs)
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[List[str]] = None) -> None:
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.unilora_indices_A:
                base_layer = self.get_base_layer()
                if safe_merge:
    ...
    def _get_effective_theta_D(self, adapter: str, device: torch.device, cast_to_fp32: bool = False) -> torch.Tensor:
        theta_d = self.unilora_multi_hashing_theta_d[adapter].to(device)
        projection = self.unilora_multi_hashing_P[adapter].to(device)
        if cast_to_fp32:
            theta_d = theta_d.float()
            projection = projection.float()
        return (projection * theta_d).sum(dim=0)
    ...
    def _get_lora_matrices(self, adapter: str, cast_to_fp32: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        theta_D = self._get_effective_theta_D(adapter, indices_A.device, cast_to_fp32=cast_to_fp32)
        A = theta_D[indices_A.long()] * self.unilora_scales_A[adapter]
        B = theta_D[indices_B.long()] * self.unilora_scales_B[adapter]
        if cast_to_fp32:
            A = A.float()
            B = B.float()
        return A, B
    ...
    def get_delta_weight(self, adapter: str) -> torch.Tensor:
        ...
        dtype = self.unilora_multi_hashing_theta_d[adapter].dtype
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                if active_adapter not in self.unilora_indices_A:
                    continue
                A, B = self._get_lora_matrices(active_adapter)
                x_cast = x.to(self.unilora_multi_hashing_theta_d[active_adapter].dtype)
