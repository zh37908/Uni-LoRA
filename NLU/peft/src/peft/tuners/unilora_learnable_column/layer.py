...
class UniLoRALayer(BaseTunerLayer):
    # Scales are global learnable parameters shared by column index.
    adapter_layer_names = ("unilora_theta_d", "unilora_scales")
    ...
    def __init__(self, base_layer: nn.Module, **kwargs):
        ...
        # Column indices for shared column scales (non-trainable buffers).
        self.unilora_col_indices_A = BufferDict({}, persistent=True)
        self.unilora_col_indices_B = BufferDict({}, persistent=True)
    ...
    def update_layer(
        ...
        # Store references to shared parameter banks.
        self.unilora_theta_d = unilora_theta_d
        self.unilora_scales = unilora_scales
    ...
    def reset_unilora_parameters(self, adapter_name, theta_d_length):
        ...
        Column indices are initialized as placeholders and will be replaced by global allocation.
        """
        if adapter_name in self.unilora_theta_d.keys():
            ...
            # Placeholders for column indices (overwritten by the model).
            self.unilora_col_indices_A[adapter_name] = torch.zeros(self.in_features, dtype=torch.long)
            self.unilora_col_indices_B[adapter_name] = torch.zeros(self.r[adapter_name], dtype=torch.long)
    ...
    def update_norm(
        ...
        """
        Updates column index buffers for shared column scales.
        """
        if adapter_name in self.unilora_theta_d.keys():
            base_layer = self.get_base_layer()
            target_device = base_layer.weight.device

            self.unilora_col_indices_A[adapter_name] = col_indices_A.to(device=target_device, dtype=torch.long)
            self.unilora_col_indices_B[adapter_name] = col_indices_B.to(device=target_device, dtype=torch.long)
...
class Linear(nn.Linear, UniLoRALayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(
            adapter_name, unilora_theta_d, unilora_scales, r, theta_d_length, unilora_dropout,
        )
    ...
    def _get_lora_matrices(self, adapter, cast_to_fp32=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        unilora_scales = self.unilora_scales[adapter].to(unilora_indices_A.device)

        if cast_to_fp32:
            unilora_theta_d = unilora_theta_d.float()
            unilora_scales = unilora_scales.float()

        # Column-wise shared scale factors.
        col_idx_A = self.unilora_col_indices_A[adapter].long()
        col_idx_B = self.unilora_col_indices_B[adapter].long()

        scale_A = unilora_scales[col_idx_A].unsqueeze(0)
        scale_B = unilora_scales[col_idx_B].unsqueeze(0)

        A = unilora_theta_d[unilora_indices_A.long()] * scale_A
        B = unilora_theta_d[unilora_indices_B.long()] * scale_B
