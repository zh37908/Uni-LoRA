...
@dataclass
class UniLoRARoSAGlobalConfig(PeftConfig):
    """
    Global UniLoRA-RoSA variant.

    All low-rank A/B entries together with RoSA's sparse matrix entries are
    conceptually concatenated into one global vector ``theta_D`` and generated
    from a shared low-dimensional bank ``theta_d`` through a fixed projection
    layout ``theta_D = P * theta_d``.
    """

    r: int = field(default=4, metadata={"help": "The rank of incremental matrices."})
    proj_seed: int = field(default=42, metadata={"help": "Random seed for global projection/index generation."})
    theta_d_length: int = field(default=256, metadata={"help": "Total theta_d length."})
    target_modules: Optional[Union[List[str], str]] = field(
        default=None,
        metadata={
            "help": (
                "List of module names or regex expression of the module names to replace with UniLoRA-RoSA-Global. "
    ...
    bias: str = field(default="none", metadata={"help": "Bias type for UniLoRA-RoSA-Global adapters."})
    ...
    def __post_init__(self):
        self.peft_type = PeftType.UNILORA_ROSA_GLOBAL
        self.target_modules = set(self.target_modules) if isinstance(self.target_modules, list) else self.target_modules

        if self.r <= 0:
            raise ValueError("`r` must be a positive integer for UniLoRA-RoSA-Global.")
        if self.theta_d_length <= 0:
            raise ValueError("`theta_d_length` must be positive for UniLoRA-RoSA-Global.")
