...
# Use absolute import or relative import based on the structure
from peft.utils.other import transpose
from .._buffer_dict import BufferDict

class UniLoRALayer(BaseTunerLayer):
    # List all names of layers that may contain adapter weights
    # unilora_scales_A and unilora_scales_B are now learnable parameters
    adapter_layer_names = ("unilora_theta_d", "unilora_scales_A", "unilora_scales_B")
    ...
    def __init__(self, base_layer: nn.Module, **kwargs):
        ...
        # Indices remain as non-trainable buffers
        self.unilora_indices_A = BufferDict({}, persistent=True)
        self.unilora_indices_B = BufferDict({}, persistent=True)
        
        # Scaling factors are now learnable parameters
        self.unilora_scales_A = nn.ParameterDict({})
        self.unilora_scales_B = nn.ParameterDict({})
    ...
    def update_norm(
        ...
        """
        Updates the scaling factors. In this variant, they are learnable parameters.
        """
        if adapter_name in self.unilora_theta_d.keys():
            base_layer = self.get_base_layer()
            ...
            # Wrap in nn.Parameter to make them learnable
            self.unilora_scales_A[adapter_name] = nn.Parameter(unilora_scales_A.to(
                device=target_device, dtype=target_dtype
            ))
            self.unilora_scales_B[adapter_name] = nn.Parameter(unilora_scales_B.to(
                device=target_device, dtype=target_dtype
            ))
...
class Linear(nn.Linear, UniLoRALayer):
    ...
    def _get_lora_matrices(self, adapter, cast_to_fp32=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Scales are now learnable Parameters.
