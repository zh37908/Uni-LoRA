...
class UniLoRAGoRAModel(BaseTuner):
    """
    UniLoRA-GoRA model: UniLoRA with per-module rank allocation.
    """

    prefix: str = "unilora_gora_"
    tuner_layer_cls = UniLoRAGoRALayer
    ...
    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        ...
        for module in model.named_modules():
            _, layer = module
            if isinstance(layer, UniLoRAGoRALayer):
        ...
        for module in model.modules():
            if isinstance(module, UniLoRAGoRALayer):
        ...
        if pointer != len(all_elements):
            raise RuntimeError("UniLoRA-GoRA index assignment is inconsistent.")
        ...
        unilora_modules = [m for m in self.modules() if isinstance(m, UniLoRAGoRALayer)]
    ...
    def _init_unilora_theta_d(self, config: UniLoRAGoRAConfig, adapter_name: str) -> None:
        if adapter_name in self.unilora_gora_theta_d:
            return
        ...
        self.unilora_gora_theta_d[adapter_name] = unilora_theta_d

    def _pre_injection_hook(self, model: nn.Module, config: UniLoRAGoRAConfig, adapter_name: str) -> None:
        self.unilora_gora_theta_d = nn.ParameterDict({})
    ...
    def _create_and_replace(
        ...
        rank_map = getattr(unilora_config, "gora_rank_map", None)
        resolved_rank = self._resolve_rank_from_map(rank_map, current_key, unilora_config.r)

        if isinstance(target, Linear):
            target.update_layer(
                adapter_name=adapter_name,
                unilora_theta_d=self.unilora_gora_theta_d,
                r=resolved_rank,
    ...
    def get_nb_savable_parameters(self, adapter="default") -> tuple[int, int]:
        ...
        for name, param in self.named_parameters():
            if "unilora_gora_theta_d" in name:
    ...
    def print_savable_parameters(self) -> None:
        ...
        print(
            f"UniLoRA-GoRA params to-be-saved (float32-equivalent): {theta_d_params:,d} "
