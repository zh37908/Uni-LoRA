...
class UniLoRARoSACompressionLayer(BaseTunerLayer):
    """
    UniLoRA-RoSA-Compression:
    - base (dense) part: theta_d[indices] * scales
    - sparse (RoSA) part: sparse_theta_d[compressed_indices] * sparse_mask[offsets]
    """
    ...
    other_param_names = (
        ...
        "unilora_rosa_sparse_indices_A",
        "unilora_rosa_sparse_indices_B",
        "unilora_rosa_sparse_scales_A",
        "unilora_rosa_sparse_scales_B",
    )
    ...
    def __init__(self, base_layer: nn.Module, **kwargs):
        ...
        # Dense->offsets mapping (per A/B entry into the flattened lora_para_cnt)
        self.unilora_theta_D_offsets_A = BufferDict({}, persistent=True)
        self.unilora_theta_D_offsets_B = BufferDict({}, persistent=True)

        # Sparse compression indices + scales (map A/B entries to sparse bank indices)
        self.unilora_rosa_sparse_indices_A = BufferDict({}, persistent=True)
        self.unilora_rosa_sparse_indices_B = BufferDict({}, persistent=True)
        self.unilora_rosa_sparse_scales_A = BufferDict({}, persistent=True)
        self.unilora_rosa_sparse_scales_B = BufferDict({}, persistent=True)
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            raise ValueError(f"Unsupported base layer type {type(base_layer)} for UniLoRARoSACompressionLayer.")
    ...
    def update_layer(
        ...
        # Initialize indices; these will be overwritten by the model init hook.
    ...
    def reset_unilora_parameters(self, adapter_name: str, theta_d_length: int, sparse_theta_d_length: int):
        ...
        sparse_indices_A = torch.randint(0, sparse_theta_d_length, (self.r[adapter_name], self.in_features), dtype=torch.long)
        sparse_indices_B = torch.randint(0, sparse_theta_d_length, (self.out_features, self.r[adapter_name]), dtype=torch.long)

        self.unilora_indices_A[adapter_name] = indices_A
        self.unilora_indices_B[adapter_name] = indices_B

        self.unilora_rosa_sparse_indices_A[adapter_name] = sparse_indices_A
        self.unilora_rosa_sparse_indices_B[adapter_name] = sparse_indices_B
    ...
    def update_sparse_norm(
        self,
        adapter_name: str,
        unilora_rosa_sparse_scales_A: torch.Tensor,
        unilora_rosa_sparse_scales_B: torch.Tensor,
    ):
        if adapter_name in self.unilora_rosa_sparse_theta_d:
            base_layer = self.get_base_layer()
            target_device = base_layer.weight.device
            target_dtype = base_layer.weight.dtype
            self.unilora_rosa_sparse_scales_A[adapter_name] = unilora_rosa_sparse_scales_A.to(
                device=target_device, dtype=target_dtype
            )
            self.unilora_rosa_sparse_scales_B[adapter_name] = unilora_rosa_sparse_scales_B.to(
                device=target_device, dtype=target_dtype
            )
...
class Linear(nn.Linear, UniLoRARoSACompressionLayer):
    def __init__(
        self,
        base_layer,
        unilora_rosa_theta_d,
        ...
        **kwargs,
        ...
        UniLoRARoSACompressionLayer.__init__(self, base_layer, **kwargs)
    ...
    def _get_lora_matrices(self, adapter: str, cast_to_fp32: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        sparse_indices_A = self.unilora_rosa_sparse_indices_A[adapter]
        sparse_indices_B = self.unilora_rosa_sparse_indices_B[adapter]

        scales_A = self.unilora_scales_A[adapter]
        scales_B = self.unilora_scales_B[adapter]
        sparse_scales_A = self.unilora_rosa_sparse_scales_A[adapter]
        sparse_scales_B = self.unilora_rosa_sparse_scales_B[adapter]
        ...
        if cast_to_fp32:
            ...
            sparse_scales_A = sparse_scales_A.float()
            sparse_scales_B = sparse_scales_B.float()

        base_A = theta_d[indices_A.long()] * scales_A
        base_B = theta_d[indices_B.long()] * scales_B

        # Per-offset mask gating:
        # - sparse_mask is defined over flattened parameter offsets (len=lora_para_cnt)
        # - sparse_theta_d is a compressed bank; we index it via sparse_indices_{A,B}
        mask_A = sparse_mask[offsets_A.long()].to(dtype=sparse_theta_d.dtype)
        mask_B = sparse_mask[offsets_B.long()].to(dtype=sparse_theta_d.dtype)

        sparse_A = sparse_theta_d[sparse_indices_A.long()] * sparse_scales_A * mask_A
        sparse_B = sparse_theta_d[sparse_indices_B.long()] * sparse_scales_B * mask_B
