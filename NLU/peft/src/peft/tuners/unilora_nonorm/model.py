...
# 假设你有这个映射，如果没有可以像 VBLoRA 那样导入或定义
from peft.utils import TRANSFORMERS_MODELS_TO_LORA_TARGET_MODULES_MAPPING 
from .config import UniLoRANonormConfig
from .layer import Linear, UniLoRALayer

class UniLoRANonormModel(BaseTuner):
    """
    Creates UniLoRA model from a pretrained transformers model.
    """
    prefix: str = "unilora_nonorm_"
    # 指定 Layer 类，BaseTuner 需要用它来识别层
    tuner_layer_cls = UniLoRALayer 
    # 如果你有 UniLoRA 特定的映射，请替换这里，否则通常复用 LoRA 的映射
    target_module_mapping = TRANSFORMERS_MODELS_TO_LORA_TARGET_MODULES_MAPPING

    def __init__(self, model, config, adapter_name, low_cpu_mem_usage: bool = False) -> None:
        # BaseTuner 的 init 会调用 _create_and_replace 注入层
        super().__init__(model, config, adapter_name, low_cpu_mem_usage=low_cpu_mem_usage)
        
        # --- UniLoRA 特有的初始化逻辑 (全局哈希索引分配) ---
        # 1. 统计所需的总索引数量 (使用新的 indices 变量)
        LoRA_para_cnt = 0
        ...
        # 4. 计算每个索引的使用频率，用于归一化 (Scales)
        counts = torch.bincount(all_elements, minlength=theta_d_length) 
        # No normalization for unilora_nonorm
        sqrt_counts = torch.ones_like(counts.float()) 
    ...
    def _init_unilora_theta_d(self, config: UniLoRANonormConfig, adapter_name: str) -> None:
        ...
        self.unilora_nonorm_theta_d[adapter_name] = unilora_theta_d

    def _pre_injection_hook(self, model: nn.Module, config: UniLoRANonormConfig, adapter_name: str) -> None:
        self.unilora_nonorm_theta_d = nn.ParameterDict({})
    ...
    def _create_and_replace(
        ...
        if isinstance(target, Linear):
            target.update_layer(
                ...
                unilora_theta_d=self.unilora_nonorm_theta_d,
                r=unilora_config.r,
    ...
    def get_nb_savable_parameters(self, adapter="default") -> tuple[int, int]:
        ...
        for name, param in self.named_parameters():
            if "unilora_nonorm_theta_d" in name:
                theta_d_params += param.numel()
            # 【修改】更新为检查 indices 和 scales
