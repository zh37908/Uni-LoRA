...
#SBATCH --output=logs/theory_e2_stsb_noise_%j.out
#SBATCH --error=logs/theory_e2_stsb_noise_%j.err

# E2 (primary variant): STS-B additive Gaussian label noise, y_tilde = y + N(0, tau^2).
# Zero-mean noise keeps the population optimum unchanged while inflating estimation noise,
# so it is the clean sigma_eff^2 sweep for Proposition 1 (theory predicts
# Delta(tau) ~ c0 + c1*tau^2 with c1 > 0). All methods share the noise via --label_noise_seed.
# Override examples:
#   LABEL_NOISE_STDS="0.0 0.5" sbatch submit_theory_e2_stsb_noise.sh
...
# STS-B labels live in [0, 5]; tau up to 1.0 is a substantial but not degenerate noise level.
LABEL_NOISE_STDS=(${LABEL_NOISE_STDS:-0.0 0.25 0.5 1.0})
LABEL_NOISE_SEED="${LABEL_NOISE_SEED:-12345}"

BATCH_SIZE="${BATCH_SIZE:-32}"
RANK="${RANK:-4}"
HEAD_LR="${LR_STSB:-2e-4}"
UNILORA_THETA_D="${UNILORA_THETA_D:-23040}"
THETA_D_LR="${THETA_D_LR:-5e-3}"
INIT_THETA_D_BOUND="${INIT_THETA_D_BOUND:-0.02}"
PROLOSA_TD="${PROLOSA_TD:-22320}"
PROLOSA_SB="${PROLOSA_SB:-720}"
...
OUT_ROOT="${OUT_ROOT:-results_theory_e2_stsb_noise}"
mkdir -p "${OUT_ROOT}"
...
seed = """${4}"""
tau = float("""${5}""")
stem = f"{variant}_stsb_{model}_lr{lr}_seed{seed}"
if tau > 0.0:
    stem += f"_tau{tau:g}"
...
for TAU in "${LABEL_NOISE_STDS[@]}"; do
  TAU_TAG="$(tau_tag "${TAU}")"
  for METHOD in "${METHODS[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
      SEED_DIR="${OUT_ROOT}/${MODEL}/${TASK}/tau${TAU_TAG}/${METHOD_NAME}/seed_${SEED}"
      mkdir -p "${SEED_DIR}"
      LOG_FILE="${SEED_DIR}/log_lr_${HEAD_LR}.txt"
      RESULT_JSON="$(result_json_path "${SEED_DIR}" "${METHOD}" "${HEAD_LR}" "${SEED}" "${TAU}")"
      ...
      FULL_CMD="srun --ntasks=1 --nodes=1 --exclusive --gres=gpu:1 --cpus-per-task=16 --cpu-bind=none --gpu-bind=single:1 python ${SCRIPT} --variant ${METHOD} --model_name ${MODEL} --task ${TASK} --batch_size ${BATCH_SIZE} --rank ${RANK} --head_lr ${HEAD_LR} --seed ${SEED} --out_dir ${SEED_DIR} --label_noise_std ${TAU} --label_noise_seed ${LABEL_NOISE_SEED}"
      if [[ "${METHOD}" == "lora" ]]; then
...
echo ">>> Generated ${TOTAL_RUNS} E2 STS-B additive-noise jobs."
echo ">>> methods=${METHODS[*]} seeds=${SEEDS[*]} taus=${LABEL_NOISE_STDS[*]}"
echo ">>> model=${MODEL} rank=${RANK} head_lr=${HEAD_LR} label_noise_seed=${LABEL_NOISE_SEED} out_root=${OUT_ROOT}"

if [[ "${TOTAL_RUNS}" -gt 0 ]]; then
  echo ">>> Starting parallel queue with ${PARALLEL_JOBS} slots..."
  xargs -I {} -P "${PARALLEL_JOBS}" bash -c "{}" < "${CMD_LIST}"
else
  echo "No new E2 STS-B jobs to run."
fi

rm -f "${CMD_LIST}"
echo "All E2 STS-B additive-noise jobs have been processed."
