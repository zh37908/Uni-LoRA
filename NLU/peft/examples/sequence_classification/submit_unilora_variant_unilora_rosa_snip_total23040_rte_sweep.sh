#!/bin/bash
#SBATCH --job-name=unilora_rosa_snip_rte_sweep
#SBATCH --nodes=1
...
#SBATCH --output=logs/unilora_rosa_snip_total23040_rte_sweep_%j.out
#SBATCH --error=logs/unilora_rosa_snip_total23040_rte_sweep_%j.err
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_snip_total23040_rte_sweep}"
mkdir -p "${OUT_ROOT}"
...
#
# Rationale:
# - plain UniLoRA roberta-large RTE prefers head_lr around 5e-3, with 1e-3/2e-3 nearby.
# - previous UniLoRA-RoSA all-task RTE was strong at sb=1440,w=64,slrm=0.2.
# - current RoSA-SNIP sb=720,w=128,slrm=0.2,sdecay=1 underperformed on RTE.
CONFIGS=(
  "1440 64 1 5e-3 0.2 1 5e-3 0"
  "1440 64 1 5e-3 0.2 1 5e-3 1"
  "1440 64 1 5e-3 0.1 1 5e-3 0"
  "1440 32 1 5e-3 0.2 1 5e-3 0"
  "1440 128 1 5e-3 0.2 1 5e-3 0"
  "1080 64 1 5e-3 0.2 1 5e-3 0"
  "1080 64 1 5e-3 0.1 1 5e-3 0"
  "1440 64 1 2e-3 0.1 1 5e-3 0"
  "1440 64 1 5e-3 0.2 1 2e-3 0"
  "1440 64 1 5e-3 0.2 1 1e-3 0"
)

echo ">>> Pre-warming cache (downloading model and RTE dataset if needed)..."
python - <<PY
...
echo ">>> Generated ${TOTAL_RUNS} UniLoRA-RoSA-SNIP RTE sweep jobs."
...
rm -f "${CMD_LIST}"
echo "All total-budget UniLoRA-RoSA-SNIP RTE sweep jobs have been processed."
