#!/bin/bash
#
# Local / ACF-style sequential runner for UniLoRA-RoSA-Stage on CoLA,
# with the same fixed total trainable budget setup as
# `acf_submit_unilora_variant_unilora_rosa_total23040_cola.sh`.
#
# The sparse adapter is activated after 50% of the full training epochs by
# default via `--rosa_stage_ratio 0.5`. Its LR stays at the configured base
# value until activation, then decays over the remaining training steps.
#
set -euo pipefail
...
# Fixed setting for the 5-seed CoLA run.
SPARSE_BUDGET_LIST=(${SPARSE_BUDGET_LIST:-720})

# Start collecting sparse-mask gradients after this fraction of total epochs.
ROSA_STAGE_RATIO_LIST=(${ROSA_STAGE_RATIO_LIST:-0.5})
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_stage_total23040_cola_sparse_decay_acf}"
mkdir -p "${OUT_ROOT}"
          ...
          for ROSA_MASK_STEPS in "${ROSA_MASK_STEPS_LIST[@]}"; do
            for ROSA_SPARSE_MULT in "${ROSA_SPARSE_LR_MULT_LIST[@]}"; do
              for ROSA_RESET_OPTIMIZER_ON_MASK in "${ROSA_RESET_LIST[@]}"; do
                ...
                METHOD_NAME="${VARIANT}_tp${TOTAL_TRAINABLE_BUDGET}_td${THETA_D_LENGTH}_sb${SPARSE_BUDGET}_d${ROSA_DENSITY}_sr${STAGE_TAG}_m${ROSA_MASK_STEPS}_slrm${MULT_TAG}_rst${ROSA_RESET_OPTIMIZER_ON_MASK}_sdecay${ROSA_DECAY_SPARSE_LR_AFTER_ACTIVATION}"
...
echo ">>> Running ${TOTAL_RUNS} UniLoRA-RoSA-Stage total-budget jobs sequentially on local GPU ${GPU}"
...
echo ">>> sparse_budget_list=${SPARSE_BUDGET_LIST[*]} stage_ratio_list=${ROSA_STAGE_RATIO_LIST[*]}"
echo ">>> mask_steps_list=${ROSA_MASK_STEPS_LIST[*]} sparse_lr_mult_list=${ROSA_SPARSE_LR_MULT_LIST[*]} reset_list=${ROSA_RESET_LIST[*]}"
echo ">>> decay_sparse_lr_after_activation=${ROSA_DECAY_SPARSE_LR_AFTER_ACTIVATION}"
echo ">>> head_lrs=${LRS[*]}"
          ...
          for ROSA_MASK_STEPS in "${ROSA_MASK_STEPS_LIST[@]}"; do
            for ROSA_SPARSE_MULT in "${ROSA_SPARSE_LR_MULT_LIST[@]}"; do
              for ROSA_RESET_OPTIMIZER_ON_MASK in "${ROSA_RESET_LIST[@]}"; do
                ...
                echo "theta_d_length=${THETA_D_LENGTH} sparse_budget=${SPARSE_BUDGET} density=${ROSA_DENSITY} stage_ratio=${ROSA_STAGE_RATIO} sparse_lr=${ROSA_SPARSE_LR} sparse_lr_decay=${ROSA_DECAY_SPARSE_LR_AFTER_ACTIVATION}"
                echo "log: ${LOG_FILE}"
                echo "=================================================="
...
echo "All total-budget UniLoRA-RoSA-Stage jobs have been processed."
