...
#
# RoSA 原文 / 官方 peft-rosa（https://github.com/IST-DASLab/peft-rosa）说明：
#   - spa_num_grads=1 为默认：用 1 个 batch 的梯度做 mask（对应本脚本的 rosa_mask_steps=1）。
#   - schedule=wl64：低秩先训 64 步再收集稀疏 mask 的梯度（对应 rosa_warmup_steps=64）。
#   - 官方还用 grad_acc_mode=mean_squared、并在 mask 后常「重启训练」；我们这边用 rosa_reset_optimizer_on_mask 近似，
#     但也会扫关闭重置，避免 Adam 状态被清空导致断崖。
#
# 超参网格可通过环境变量覆盖；默认在「可承受」规模下多试几组 mask / warmup / 稀疏相对学习率 / 是否重置优化器。
#
set -euo pipefail
...
# 对应 peft-rosa 的 spa_num_grads：用多少个 optimizer step 累计 |grad|（我们实现为 max 累积）
ROSA_MASK_STEPS_LIST=(${ROSA_MASK_STEPS_LIST:-1 8})
# 低秩-only warmup 步数；可试更长以稳定 mask（官方示例 wl64）
ROSA_WARMUP_STEPS_LIST=(${ROSA_WARMUP_STEPS_LIST:-64 256})
# 稀疏组学习率 = MULT * THETA_D_LR（避免 S 与 theta_d 同量级过猛）；1.0 即与 theta_d 相同
ROSA_SPARSE_LR_MULT_LIST=(${ROSA_SPARSE_LR_MULT_LIST:-0.2 1.0})
# 1 = mask 生成后 --rosa_reset_optimizer_on_mask；0 = 不重置（保留 Adam 动量）
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} UniLoRA-RoSA jobs sequentially on local GPU ${GPU}"
          ...
          for ROSA_MASK_STEPS in "${ROSA_MASK_STEPS_LIST[@]}"; do
            for ROSA_SPARSE_MULT in "${ROSA_SPARSE_LR_MULT_LIST[@]}"; do
              for ROSA_RESET_OPTIMIZER_ON_MASK in "${ROSA_RESET_LIST[@]}"; do
                ...
                METHOD_NAME="${VARIANT}_d${ROSA_DENSITY}_w${ROSA_WARMUP_STEPS}_m${ROSA_MASK_STEPS}_slrm${MULT_TAG}_rst${ROSA_RESET_OPTIMIZER_ON_MASK}"
...
echo "All local UniLoRA-RoSA jobs have been processed."
