#!/bin/bash
#
# Re-run the best UniLoRA-RoSA CoLA hyperparameter setting across 5 seeds.
# The default output root is separated from the search directory so seed_0 is
# re-executed instead of being skipped by existing results.
#
set -euo pipefail
...
# Best current CoLA search result under results_glue_variants_unilora_rosa_total23040_acf:
#   unilora_rosa_tp23040_td21600_sb1440_d0.000813802083_w64_m1_slrm0p2_rst1
#   seed_0 matthews_correlation = 0.715805685622
LRS=(${LRS:-5e-3})
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_total23040_best5seeds_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} best-setting UniLoRA-RoSA jobs sequentially on local GPU ${GPU}"
...
echo "All best-setting UniLoRA-RoSA jobs have been processed."
