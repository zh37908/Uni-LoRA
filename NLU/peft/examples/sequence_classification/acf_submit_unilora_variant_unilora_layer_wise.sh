...
# Paper-style local single-GPU config for the layer-wise ablation.
GPU="${GPU:-0}"
...
VARIANT="unilora_layer_wise"
OUT_ROOT="${OUT_ROOT:-results_glue_variants_layer_wise_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} LayerWise jobs sequentially on local GPU ${GPU}"
echo ">>> Paper-style setup: model=${MODEL} tasks=${TASKS[*]} seeds=${SEEDS[*]} batch_size=${BATCH_SIZE} rank=${RANK} theta_d_length=${THETA_D_LENGTH} theta_d_lr=${THETA_D_LR}"
...
for TASK in "${TASKS[@]}"; do
  ...
  for SEED in "${SEEDS[@]}"; do
    SEED_DIR="${TASK_DIR}/${VARIANT}/seed_${SEED}"
    mkdir -p "${SEED_DIR}"
...
echo "All local LayerWise jobs have been processed."
