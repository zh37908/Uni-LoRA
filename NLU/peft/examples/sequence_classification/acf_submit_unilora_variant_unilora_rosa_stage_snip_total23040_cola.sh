#!/bin/bash
#
# Local / ACF-style sequential runner for UniLoRA-RoSA-Stage-SNIP on CoLA,
# with the same fixed total trainable budget setup as
# `acf_submit_unilora_variant_unilora_rosa_stage_total23040_cola.sh`.
#
# The sparse adapter is activated after 50% of the full training epochs by
# default via `--rosa_stage_ratio 0.5`. Sparse positions are selected with the
# SNIP-style |W_ij * g_ij| score instead of raw gradient magnitude.
#
set -euo pipefail
...
# Start collecting sparse-mask SNIP scores after this fraction of total epochs.
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_stage_snip_total23040_cola_sparse_decay_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} UniLoRA-RoSA-Stage-SNIP total-budget jobs sequentially on local GPU ${GPU}"
          ...
          for ROSA_MASK_STEPS in "${ROSA_MASK_STEPS_LIST[@]}"; do
            for ROSA_SPARSE_MULT in "${ROSA_SPARSE_LR_MULT_LIST[@]}"; do
              for ROSA_RESET_OPTIMIZER_ON_MASK in "${ROSA_RESET_LIST[@]}"; do
                ...
                echo "theta_d_length=${THETA_D_LENGTH} sparse_budget=${SPARSE_BUDGET} density=${ROSA_DENSITY} stage_ratio=${ROSA_STAGE_RATIO} sparse_lr=${ROSA_SPARSE_LR} sparse_lr_decay=${ROSA_DECAY_SPARSE_LR_AFTER_ACTIVATION} score=snip_abs_weight_grad"
                echo "log: ${LOG_FILE}"
                echo "=================================================="
...
echo "All total-budget UniLoRA-RoSA-Stage-SNIP jobs have been processed."
