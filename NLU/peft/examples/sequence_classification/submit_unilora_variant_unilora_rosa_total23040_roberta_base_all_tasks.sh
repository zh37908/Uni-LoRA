#!/bin/bash
#SBATCH --job-name=unilora_rosa_rb_tp23040_all
#SBATCH --nodes=1
...
#SBATCH --output=logs/unilora_rosa_total23040_roberta_base_all_%j.out
#SBATCH --error=logs/unilora_rosa_total23040_roberta_base_all_%j.err
...
TASKS=(${TASKS:-cola sst2 mrpc qnli rte stsb})
SEEDS=(${SEEDS:-0 1 2 3 4})

BATCH_SIZE="${BATCH_SIZE:-32}"
RANK="${RANK:-4}"
TOTAL_TRAINABLE_BUDGET="${TOTAL_TRAINABLE_BUDGET:-23040}"
THETA_D_LENGTH="${THETA_D_LENGTH:-21600}"
SPARSE_BUDGET="${SPARSE_BUDGET:-1440}"
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_total23040_roberta_base_all_tasks}"
mkdir -p "${OUT_ROOT}"
...
task_head_lr() {
  case "${1}" in
    cola) echo "${LR_COLA:-2e-2}" ;;
    sst2) echo "${LR_SST2:-5e-4}" ;;
    mrpc) echo "${LR_MRPC:-1e-3}" ;;
    qnli) echo "${LR_QNLI:-1e-3}" ;;
    rte) echo "${LR_RTE:-1e-2}" ;;
...
echo ">>> Generated ${TOTAL_RUNS} roberta-base UniLoRA-RoSA all-task jobs."
...
echo ">>> task LRs: cola=$(task_head_lr cola), sst2=$(task_head_lr sst2), mrpc=$(task_head_lr mrpc), qnli=$(task_head_lr qnli), rte=$(task_head_lr rte), stsb=$(task_head_lr stsb)"
...
rm -f "${CMD_LIST}"
echo "All roberta-base total-budget UniLoRA-RoSA all-task jobs have been processed."
