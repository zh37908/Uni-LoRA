...
#SBATCH --output=logs/theory_e4_remaining_glue_%j.out
#SBATCH --error=logs/theory_e4_remaining_glue_%j.err

# E4/E5 GLUE remainder: RTE + STS-B full-data runs that save delta_theta.
# SST-2/QNLI come from E1 p=1; CoLA/MRPC come from E3.
...
OUT_ROOT="${OUT_ROOT:-results_theory_e4_remaining_glue}"
mkdir -p "${OUT_ROOT}"
...
task_head_lr() {
  case "${1}" in
    ...
    *)
      echo "Unsupported task for E4 remainder: ${1}" >&2
      return 1
      ;;
  esac
}
...
ROSA_DENSITY="$(python3 - <<PY
sparse_budget = int("${PROLOSA_SB}")
...
for TASK in "${TASKS[@]}"; do
  ...
  for METHOD in "${METHODS[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
      ...
      RESULT_JSON="$(result_json_path "${SEED_DIR}" "${METHOD}" "${TASK}" "${HEAD_LR}" "${SEED}")"
      ...
      FULL_CMD="srun --ntasks=1 --nodes=1 --exclusive --gres=gpu:1 --cpus-per-task=16 --cpu-bind=none --gpu-bind=single:1 python ${SCRIPT} --variant ${METHOD} --model_name ${MODEL} --task ${TASK} --batch_size ${BATCH_SIZE} --rank ${RANK} --head_lr ${HEAD_LR} --seed ${SEED} --out_dir ${SEED_DIR} --save_delta_theta"
      if [[ "${METHOD}" == "lora" ]]; then
...
echo ">>> Generated ${TOTAL_RUNS} E4 remaining-GLUE jobs."
echo ">>> tasks=${TASKS[*]} methods=${METHODS[*]} seeds=${SEEDS[*]} out_root=${OUT_ROOT}"

if [[ "${TOTAL_RUNS}" -gt 0 ]]; then
  echo ">>> Starting parallel queue with ${PARALLEL_JOBS} slots..."
  xargs -I {} -P "${PARALLEL_JOBS}" bash -c "{}" < "${CMD_LIST}"
else
  echo "No new E4 jobs to run."
fi

rm -f "${CMD_LIST}"
echo "All E4 remaining-GLUE jobs have been processed."
