#!/bin/bash
#
# Local / ACF-style sequential runner for UniLoRA-Multi-Structured on GLUE.
#
# 目标：
# - 固定 multi-structured 结构（默认 num_hash_pairs=4）
...
# 扫描 theta_d_lr（multi-structured 参数组）
...
# Multi-Structured 结构参数（默认扫 M=8,9）
M_LIST=(${M_LIST:-8 10})
...
VARIANT="unilora_multi_structured"
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_multi_structured_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} UniLoRA-Multi-Structured jobs sequentially on local GPU ${GPU}"
...
echo ">>> JSON results will be saved by ${SCRIPT} into each run out_dir."
...
echo "All local UniLoRA-Multi-Structured jobs have been processed."
