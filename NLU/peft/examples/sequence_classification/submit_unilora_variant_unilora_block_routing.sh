...
# 实验参数
METHODS=(unilora_block_routing)
MODELS=(roberta-large)
TASKS=(cola  mrpc qnli rte stsb)
SEEDS=(0 1 2)
LRS=(1e-4 2e-4 5e-4 1e-3 2e-3 5e-3)

SCRIPT=run_unilora_variants_glue.py
OUT_ROOT=results_glue_variants_unilora_block_routing_8_blocks
mkdir -p "${OUT_ROOT}"
      ...
      for ALPHA in "${ALPHAS[@]}"; do
        ...
        for SEED in "${SEEDS[@]}"; do
          ...
          for LR in "${LRS[@]}"; do
            ...
            FULL_CMD="srun --ntasks=1 --nodes=1 --exclusive --gres=gpu:1 --cpus-per-task=8 --cpu-bind=none --gpu-bind=single:1 python ${SCRIPT} --variant ${METHOD} ${EXTRA_FLAG} --model_name ${MODEL} --task ${TASK} --head_lr ${LR} --seed ${SEED} --num_blocks 8 --gumbel_tau 1.0 --out_dir ${SEED_DIR} > ${LOG_FILE} 2>&1"
