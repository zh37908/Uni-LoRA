...
# 实验参数
# METHODS=(unilora_fastfood unilora_nonorm unilora_learnable unilora_learnable_column unilora_isometric_control)
METHODS=(direct_unilora)
...
OUT_ROOT=results_glue_variants_direct_unilora_td1277952
mkdir -p "${OUT_ROOT}"
      ...
      for ALPHA in "${ALPHAS[@]}"; do
        ...
        for SEED in "${SEEDS[@]}"; do
          ...
          for LR in "${LRS[@]}"; do
            ...
            FULL_CMD="srun --ntasks=1 --nodes=1 --exclusive --gres=gpu:1 --cpus-per-task=8 --gpu-bind=single:1 python ${SCRIPT} --variant ${METHOD} ${EXTRA_FLAG} --model_name ${MODEL} --task ${TASK} --head_lr ${LR} --seed ${SEED} --theta_d_length 1277952 --out_dir ${SEED_DIR} > ${LOG_FILE} 2>&1"
