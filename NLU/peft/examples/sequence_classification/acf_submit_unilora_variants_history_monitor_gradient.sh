...
MONITOR_WINDOW_SIZE="${MONITOR_WINDOW_SIZE:-32}"
MONITOR_SCOPE="${MONITOR_SCOPE:-adapter}"
...
SCRIPT=run_unilora_variants_glue_monitor_gradient.py
OUT_ROOT="${OUT_ROOT:-results_glue_variants_history_monitor_gradient_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} monitor-gradient jobs sequentially on local GPU ${GPU}"
...
echo "All local monitor-gradient jobs have been processed."
