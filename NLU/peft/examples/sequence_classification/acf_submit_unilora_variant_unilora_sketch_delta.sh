...
VARIANT="unilora_sketch_delta"
METHOD_NAME="${VARIANT}_b${SKETCH_BITS}_g${SKETCH_GROUPS_PER_ROW}"
OUT_ROOT="${OUT_ROOT:-results_glue_variants_sketch_delta_acf}"
mkdir -p "${OUT_ROOT}"
...
echo ">>> Running ${TOTAL_RUNS} SketchDelta jobs sequentially on local GPU ${GPU}"
echo ">>> Config: model=${MODEL} tasks=${TASKS[*]} seeds=${SEEDS[*]} batch_size=${BATCH_SIZE} rank=${RANK} bits=${SKETCH_BITS} groups=${SKETCH_GROUPS_PER_ROW} theta_d_lr=${THETA_D_LR}"
...
echo "All local SketchDelta jobs have been processed."
