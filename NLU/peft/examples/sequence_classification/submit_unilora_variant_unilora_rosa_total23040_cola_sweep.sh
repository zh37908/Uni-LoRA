#!/bin/bash
#SBATCH --job-name=unilora_rosa_cola_sweep
#SBATCH --nodes=1
...
#SBATCH --output=logs/unilora_rosa_total23040_cola_sweep_%j.out
#SBATCH --error=logs/unilora_rosa_total23040_cola_sweep_%j.err
...
OUT_ROOT="${OUT_ROOT:-results_glue_variants_unilora_rosa_total23040_cola_sweep}"
mkdir -p "${OUT_ROOT}"

# Focused default sweep:
#   sparse_budget warmup mask_steps theta_d_lr sparse_lr_mult reset_optimizer
#
# Rationale:
# - The all-task setting (sb=1440, w=64, m=1, theta_d_lr=5e-3, slrm=0.2)
#   was unstable on CoLA.
# - CoLA-specific prior scripts favored smaller sparse budgets and earlier sparse activation.
# - This default sweep isolates sparse budget and RoSA-side LR effects before expanding.
CONFIGS=(
  "720 32 1 2e-3 0.1 1"
  "720 64 1 2e-3 0.1 1"
  "720 128 1 2e-3 0.1 1"
  "720 32 1 5e-3 0.2 1"
  "720 64 1 5e-3 0.2 1"
  "1440 64 1 2e-3 0.1 1"
)

# Optional broader mode:
#   BROAD_SWEEP=1 sbatch submit_unilora_variant_unilora_rosa_total23040_cola_sweep.sh
if [[ "${BROAD_SWEEP:-0}" == "1" ]]; then
  CONFIGS=()
  for SPARSE_BUDGET in 360 720 1440; do
    for ROSA_WARMUP_STEPS in 32 64 128; do
      for ROSA_MASK_STEPS in 1 4; do
        for THETA_D_LR in 2e-3 5e-3; do
          for ROSA_SPARSE_LR_MULT in 0.1 0.2; do
            CONFIGS+=("${SPARSE_BUDGET} ${ROSA_WARMUP_STEPS} ${ROSA_MASK_STEPS} ${THETA_D_LR} ${ROSA_SPARSE_LR_MULT} 1")
          done
        done
      done
    done
...
for CONFIG in "${CONFIGS[@]}"; do
  read -r SPARSE_BUDGET ROSA_WARMUP_STEPS ROSA_MASK_STEPS THETA_D_LR ROSA_SPARSE_LR_MULT ROSA_RESET_OPTIMIZER_ON_MASK <<< "${CONFIG}"
  ...
  TDLR_TAG="${TDLR_TAG//-e/-}"
  METHOD_NAME="${VARIANT}_tp${TOTAL_TRAINABLE_BUDGET}_td${THETA_D_LENGTH}_sb${SPARSE_BUDGET}_d${ROSA_DENSITY}_w${ROSA_WARMUP_STEPS}_m${ROSA_MASK_STEPS}_tdlr${TDLR_TAG}_slrm${MULT_TAG}_rst${ROSA_RESET_OPTIMIZER_ON_MASK}"
...
echo ">>> Generated ${TOTAL_RUNS} CoLA UniLoRA-RoSA sweep jobs."
...
rm -f "${CMD_LIST}"
echo "All CoLA UniLoRA-RoSA sweep jobs have been processed."
