...
def parse_args():
    ...
    parser.add_argument("--lora_lr", type=float, default=None, help="Optional separate LR for LoRA A/B; defaults to --head_lr.")
    parser.add_argument("--lora_alpha", type=int, default=None, help="LoRA alpha; defaults to rank for backward compatibility.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Explicit LR warmup optimizer steps; overrides --warmup_ratio.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Exact sparse branch budget for LoRA-RoSA; overrides --rosa_density when set.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rosa_grad_acc_mode",
        choices=["max_abs", "mean_abs", "mean_squared"],
        default="max_abs",
        help="Aggregation used for LoRA-RoSA base-weight mask scores.",
    )
    parser.add_argument("--rosa_mask_save_path", type=str, default=None, help="Write a generated LoRA-RoSA mask here.")
    parser.add_argument("--rosa_mask_load_path", type=str, default=None, help="Load a fixed LoRA-RoSA mask before training.")
    parser.add_argument(
        "--rosa_terminate_after_mask_generation",
        action="store_true",
        help="Exit successfully after generating and saving the LoRA-RoSA mask.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Fraction of the original training set to keep (stratified). Same subset_seed shares the subset across methods.",
    )
    parser.add_argument(
        "--subset_seed",
        type=int,
        default=12345,
        help="Seed used only for training-subset sampling; independent of --seed.",
    )
    parser.add_argument(
        "--label_noise_ratio",
        type=float,
        default=0.0,
        help="Fraction of training labels to flip uniformly. Dev labels stay clean.",
    )
    parser.add_argument(
        "--label_noise_seed",
        type=int,
        default=12345,
        help="Seed used only for label-noise sampling; independent of --seed.",
    )
    parser.add_argument(
        "--label_noise_std",
        type=float,
        default=0.0,
        help="Std of zero-mean additive Gaussian label noise (regression tasks only, e.g. STS-B). Dev labels stay clean.",
    )
    parser.add_argument(
        "--max_steps",
        type=int,
        default=None,
        help="Stop after this many optimizer steps. Overrides the epoch budget when set.",
    )
    parser.add_argument(
        "--fix_full_data_steps",
        action="store_true",
        help="Use the full-data epoch budget as a fixed step count, even after subsetting.",
    )
    parser.add_argument(
        "--eval_every_steps",
        type=int,
        default=None,
        help="Evaluate every N optimizer steps. Defaults to one full-data epoch when --fix_full_data_steps is set.",
    )
    parser.add_argument(
        "--save_delta_theta",
        action="store_true",
        help="Save the flattened LoRA-space update vector and projection metadata for theory analysis.",
    )
    parser.add_argument(
        "--save_eval_logits",
        action="store_true",
        help="Save validation logits from the best checkpoint for functional bias-variance analysis.",
    )
    parser.add_argument(
        "--proj_seed",
        type=int,
        default=None,
        help="Override Uni-LoRA/ProLoSA grouping seed. Defaults to --seed.",
    )

    args = parser.parse_args()
    if args.rank is None:
        args.rank = 1 if args.variant == "unilora_aroma" else 4
    if args.scheduler_warmup_steps is not None and args.scheduler_warmup_steps < 0:
        parser.error("--scheduler_warmup_steps must be non-negative.")
    if args.rosa_terminate_after_mask_generation and not args.rosa_mask_save_path:
        parser.error("--rosa_terminate_after_mask_generation requires --rosa_mask_save_path.")
    if args.rosa_terminate_after_mask_generation and args.rosa_mask_load_path:
        parser.error("Mask generation and mask loading must run in separate processes.")
    if (args.rosa_mask_save_path or args.rosa_mask_load_path) and not args.variant.startswith("lora_rosa"):
        parser.error("LoRA-RoSA mask files are supported only by lora_rosa variants.")
    return args
...
def evaluate_glue_model(model, eval_loader, task, metric_name, device, return_logits=False):
    model.eval()
    ...
    if return_logits:
        eval_logits = torch.cat(logit_chunks, dim=0) if logit_chunks else torch.empty(0)
        return avg_eval_loss, eval_results, score, eval_logits
    return avg_eval_loss, eval_results, score


def subsample_train_dataset(train_dataset, task, train_subset_ratio, subset_seed):
    if train_subset_ratio is None or train_subset_ratio >= 1.0:
        return train_dataset, len(train_dataset), 1.0
    ...
    if target_size >= original_size:
        return train_dataset, original_size, 1.0
    ...
    if task != "stsb" and "label" in train_dataset.column_names:
        ...
        sampled_train = train_dataset.shuffle(seed=subset_seed).select(range(target_size))
    return sampled_train, original_size, len(sampled_train) / float(original_size)


def apply_label_noise(train_dataset, task, label_noise_ratio, label_noise_seed):
    if label_noise_ratio is None or label_noise_ratio <= 0.0:
        return train_dataset, 0
    if task == "stsb" or "label" not in train_dataset.column_names:
        raise ValueError(f"Label noise is only supported for classification tasks, got task={task}")
    if not 0.0 < label_noise_ratio <= 1.0:
        raise ValueError(f"label_noise_ratio must be in (0, 1], got {label_noise_ratio}")

    labels = np.array(train_dataset["label"])
    unique_labels = np.unique(labels)
    if unique_labels.size < 2:
        raise ValueError("Need at least two label classes to apply symmetric label noise.")

    rng = np.random.RandomState(label_noise_seed)
    n_flip = int(round(len(labels) * label_noise_ratio))
    n_flip = min(max(n_flip, 0), len(labels))
    if n_flip == 0:
        return train_dataset, 0
    ...
    noisy = labels.copy()
    for i in flip_idx:
        choices = unique_labels[unique_labels != labels[i]]
        noisy[i] = rng.choice(choices)
    return train_dataset.remove_columns(["label"]).add_column("label", noisy.tolist()), int(n_flip)


def apply_additive_label_noise(train_dataset, task, label_noise_std, label_noise_seed):
    """Zero-mean Gaussian label noise for regression tasks (E2 primary design: keeps the
    population optimum unchanged while inflating estimation noise)."""
    if label_noise_std is None or label_noise_std <= 0.0:
        return train_dataset
    if task != "stsb":
        raise ValueError(f"Additive label noise is only supported for stsb, got task={task}")
    labels = np.array(train_dataset["label"], dtype=np.float64)
    rng = np.random.RandomState(label_noise_seed)
    noisy = labels + rng.normal(0.0, label_noise_std, size=labels.shape)
    return train_dataset.remove_columns(["label"]).add_column("label", noisy.tolist())
...
def snapshot_trainable_state(model):
    ...
    for name, buf in model.named_buffers():
        if any(key in name for key in ("sparse_mask", "unilora_indices", "unilora_scales", "theta_D_offsets")):
...
@torch.no_grad()
def extract_lora_space_delta(model, adapter_name="default"):
    chunks = []
    module_names = []
    index_meta = {}
    for name, module in model.named_modules():
        A = B = None
        if hasattr(module, "_get_lora_matrices") and adapter_name in getattr(module, "r", {}):
            A, B = module._get_lora_matrices(adapter_name, cast_to_fp32=True)
            if hasattr(module, "unilora_indices_A") and adapter_name in module.unilora_indices_A:
                index_meta[name] = {
                    "indices_A": module.unilora_indices_A[adapter_name].detach().cpu().clone(),
                    "indices_B": module.unilora_indices_B[adapter_name].detach().cpu().clone(),
                }
                # Offsets into the global sparse theta_D vector (ProLoSA); needed to align
                # the SNIP support with the flattened delta_theta ordering.
                if (
                    hasattr(module, "unilora_theta_D_offsets_A")
                    and adapter_name in module.unilora_theta_D_offsets_A
                ):
                    index_meta[name]["offsets_A"] = (
                        module.unilora_theta_D_offsets_A[adapter_name].detach().cpu().clone()
                    )
                    index_meta[name]["offsets_B"] = (
                        module.unilora_theta_D_offsets_B[adapter_name].detach().cpu().clone()
                    )
        elif hasattr(module, "lora_A") and isinstance(getattr(module, "lora_A", None), nn.ModuleDict):
            if adapter_name in module.lora_A:
                A = module.lora_A[adapter_name].weight.detach().float()
                B = module.lora_B[adapter_name].weight.detach().float()
        if A is None or B is None:
            continue
        chunks.append(A.reshape(-1).cpu())
        chunks.append(B.reshape(-1).cpu())
        module_names.append(name)

    delta_theta = torch.cat(chunks) if chunks else torch.empty(0)
    sparse_mask = None
    for name, buf in model.named_buffers():
        if "unilora_rosa_sparse_mask" in name or name.endswith("sparse_mask"):
            sparse_mask = buf.detach().cpu().clone()
            break
    return {
        ...
        "delta_theta_norm": float(delta_theta.norm().item()) if delta_theta.numel() else 0.0,
    }
...
def build_result_json_stem(variant: str, task: str, model_name: str, head_lr, seed: int, args) -> str:
    ...
    subset_ratio = float(getattr(args, "train_subset_ratio", 1.0) or 1.0)
    noise_ratio = float(getattr(args, "label_noise_ratio", 0.0) or 0.0)
    noise_std = float(getattr(args, "label_noise_std", 0.0) or 0.0)
    extra_tags = []
    if subset_ratio < 1.0:
        extra_tags.append(f"p{subset_ratio:g}")
    if noise_ratio > 0.0:
        extra_tags.append(f"noise{noise_ratio:g}")
    if noise_std > 0.0:
        extra_tags.append(f"tau{noise_std:g}")
    if extra_tags:
        stem = f"{stem}_{'_'.join(extra_tags)}"
...
def main():
    args = parse_args()
    if args.proj_seed is None:
        args.proj_seed = args.seed
    set_seed(args.seed)
    ...
    datasets["train"], _, actual_subset_ratio = subsample_train_dataset(
        datasets["train"],
        task,
        args.train_subset_ratio,
        args.subset_seed,
    )
    datasets["train"], n_flipped_labels = apply_label_noise(
        datasets["train"],
        task,
        args.label_noise_ratio,
        args.label_noise_seed,
    )
    datasets["train"] = apply_additive_label_noise(
        datasets["train"],
        task,
        args.label_noise_std,
        args.label_noise_seed,
    )
    print(
        f"Train split: {len(datasets['train'])}/{original_train_size} examples "
        f"(requested ratio={args.train_subset_ratio:.4f}, actual ratio={actual_subset_ratio:.4f}, "
        f"subset_seed={args.subset_seed}, flipped_labels={n_flipped_labels}, "
        f"label_noise_ratio={args.label_noise_ratio:.4f}, label_noise_std={args.label_noise_std:.4f}, "
        f"label_noise_seed={args.label_noise_seed})."
    )
    ...
    if variant == "lora":
    ...
    elif variant == "lora_rosa":
        peft_config = LoraRoSAConfig(
            task_type="SEQ_CLS",
            r=args.rank,
            lora_alpha=args.lora_alpha if args.lora_alpha is not None else args.rank,
            lora_dropout=args.unilora_dropout,
            ...
            rosa_sparse_budget=args.rosa_sparse_budget,
            rosa_warmup_steps=args.rosa_warmup_steps,
            rosa_mask_steps=args.rosa_mask_steps,
            rosa_score_mode="grad",
            rosa_grad_acc_mode=args.rosa_grad_acc_mode,
            rosa_seed=args.seed,
        )
    elif variant == "lora_rosa_snip":
        peft_config = LoraRoSASnipConfig(
            task_type="SEQ_CLS",
            r=args.rank,
            lora_alpha=args.lora_alpha if args.lora_alpha is not None else args.rank,
            lora_dropout=args.unilora_dropout,
    ...
    elif variant == "lora_rosa_random":
        peft_config = LoraRoSARandomConfig(
            task_type="SEQ_CLS",
            r=args.rank,
            lora_alpha=args.lora_alpha if args.lora_alpha is not None else args.rank,
            lora_dropout=args.unilora_dropout,
        ...
        peft_config = UniLoRAAromaConfig(
            ...
            proj_seed=args.proj_seed, init_theta_d_bound=current_init_bound,
    ...
    rosa_mask_preloaded = False
    if args.rosa_mask_load_path:
        if not variant.startswith("lora_rosa") or unilora_rosa_backend is None:
            raise ValueError("--rosa_mask_load_path is supported only by lora_rosa variants.")
        if not hasattr(unilora_rosa_backend, "load_sparse_masks"):
            raise RuntimeError("This LoRA-RoSA backend does not support mask loading.")
        loaded_mask_info = unilora_rosa_backend.load_sparse_masks(
            args.rosa_mask_load_path,
            adapter_name="default",
        )
        rosa_mask_preloaded = True
        print(
            f"Loaded fixed LoRA-RoSA mask from {args.rosa_mask_load_path}: "
            f"modules={loaded_mask_info['modules']}, "
            f"selected_positions={loaded_mask_info['selected_positions']}"
        )
    ...
    head_params, lora_params, theta_d_params, sparse_params, alpha_params = [], [], [], [], []
    for n, p in model.named_parameters():
        if "lora_rosa_sparse_values" in n:
    ...
    lora_lr = args.lora_lr if args.lora_lr is not None else args.head_lr
    lora_lr_display = f"{lora_lr}" if lora_params else "N/A"
    ...
    print(
        f"  head_lr = {args.head_lr} | lora_lr = {lora_lr_display} | theta_d_lr = {theta_d_lr_display} "
    ...
    print_trainable_param_summary(
        ...
        head_params=head_params + lora_params,
    )
    if hessian_aware_backend is not None:
        print(
    ...
    if unilora_rosa_backend is not None:
        if variant in {"lora_rosa", "lora_rosa_snip", "lora_rosa_random"}:
            score_mode = {
                "lora_rosa": "max-abs base-weight gradient",
                "lora_rosa_snip": "snip |W*g|",
                "lora_rosa_random": "random",
            }[variant]
            print(
                "LoRA-RoSA sparse config: "
                ...
                f"grad_acc_mode={args.rosa_grad_acc_mode}, "
                f"mask_preloaded={rosa_mask_preloaded}, "
    ...
    if lora_params:
        optimizer_groups.append({"params": lora_params, "lr": lora_lr, "weight_decay": args.weight_decay})
    if theta_d_params:
    ...
    full_steps_per_epoch = math.ceil(original_train_size / float(batch_size))
    full_total_steps = full_steps_per_epoch * num_epochs
    use_step_budget = bool(args.fix_full_data_steps or args.max_steps is not None)
    if args.fix_full_data_steps:
    ...
    if use_step_budget:
        steps_per_epoch_now = max(1, len(train_loader))
        num_epochs = max(1, math.ceil(total_steps / steps_per_epoch_now))
        if args.eval_every_steps is None:
            args.eval_every_steps = full_steps_per_epoch
        print(
            f"Using fixed step budget: total_steps={total_steps}, "
            f"eval_every_steps={args.eval_every_steps}, expanded_epochs={num_epochs}, "
            f"full_data_steps_per_epoch={full_steps_per_epoch}, subset_steps_per_epoch={len(train_loader)}."
        )
    ...
    num_warmup_steps = (
        int(args.scheduler_warmup_steps)
        if args.scheduler_warmup_steps is not None
        else int(warmup_ratio * total_steps)
    )
    num_warmup_steps = min(total_steps, num_warmup_steps)
    ...
    if sparse_group_indices:
        ...
        elif variant.startswith("lora_rosa") and rosa_mask_preloaded:
            sparse_lr_activation_step = 0
        elif variant in {"unilora_rosa_snip", "lora_rosa", "lora_rosa_snip", "lora_rosa_random"}:
    ...
    best_score = -1e18
    best_metric = None
    best_val_loss = None  # dev loss at the best-score eval (theory main metric is dev loss)
    min_val_loss = None
    best_state = None
    history = []
    global_step = 0
    ...
    mask_generation_complete = False

    for epoch in range(num_epochs):
        model.train()
        pbar = tqdm(train_loader, desc=f"Epoch {epoch}", leave=False, disable=use_step_budget and num_epochs > 50)
        epoch_loss = 0
        epoch_batches = 0
        for batch in pbar:
            ...
            if rosa_collecting:
                ...
                if capture_stats.get("updated_tensors", 0) == 0:
                    gradient_source = "base-weight" if variant.startswith("lora_rosa") else "A/B"
                    raise RuntimeError(
                        f"{variant} did not capture any {gradient_source} gradients during sparse-mask collection."
                    )
            if hessian_aware_backend is not None:
            ...
            if (
                ...
                if args.rosa_mask_save_path:
                    if not hasattr(unilora_rosa_backend, "export_sparse_masks"):
                        raise RuntimeError("This LoRA-RoSA backend does not support mask export.")
                    saved_mask_info = unilora_rosa_backend.export_sparse_masks(
                        args.rosa_mask_save_path,
                        adapter_name="default",
                    )
                    print(
                        f"Saved fixed LoRA-RoSA mask to {args.rosa_mask_save_path}: "
                        f"modules={saved_mask_info['modules']}, "
                        f"selected_positions={saved_mask_info['selected_positions']}"
                    )
                if args.rosa_reset_optimizer_on_mask:
                    optimizer.state.clear()
                sparse_label = "LoRA-RoSA" if variant.startswith("lora_rosa") else "UniLoRA-RoSA"
                print(
                    f"Activated {sparse_label} sparse compensation: "
                ...
                mask_generation_complete = args.rosa_terminate_after_mask_generation
            if unilora_igu_backend is not None:
            ...
            global_step += 1
            reached_step_budget = use_step_budget and global_step >= total_steps
            should_eval_now = (
                args.eval_every_steps is not None
                and args.eval_every_steps > 0
                and (global_step % args.eval_every_steps == 0 or reached_step_budget)
            )
            if should_eval_now:
                avg_eval_loss, eval_results, score = evaluate_glue_model(model, eval_loader, task, metric_name, device)
                print(
                    f"Step {global_step} | train_loss: {last_train_loss:.4f} | "
                    f"val_loss: {avg_eval_loss:.4f} | {metric_name}: {score:.4f} | {eval_results}"
                )
                ...
                if min_val_loss is None or avg_eval_loss < min_val_loss:
                    min_val_loss = avg_eval_loss
                if score > best_score:
                    best_score = score
                    best_metric = eval_results
                    best_val_loss = avg_eval_loss
                    if args.save_delta_theta or args.save_eval_logits:
                        best_state = snapshot_trainable_state(model)
            if mask_generation_complete or reached_step_budget:
                break
        
        if mask_generation_complete or (use_step_budget and global_step >= total_steps):
            break
    ...
    if mask_generation_complete:
        writer.close()
        print("LoRA-RoSA mask generation phase completed; start a fresh process with --rosa_mask_load_path.")
        return
    ...
    theory_artifact = None
    eval_logits = None
    if args.save_delta_theta or args.save_eval_logits:
        if args.save_eval_logits:
            _, _, _, eval_logits = evaluate_glue_model(
                model, eval_loader, task, metric_name, device, return_logits=True
            )
        if args.save_delta_theta:
            theory_artifact = extract_lora_space_delta(model, adapter_name="default")
        # Classifier head weights let us rebuild the fine-tuned model for diagonal-Fisher
        # (curvature-weighted) analysis without saving full checkpoints.
        classifier_state = {
            k: v.detach().cpu().clone()
            for k, v in model.state_dict().items()
            if "classifier" in k
        }
        artifact_path = os.path.join(args.out_dir, f"{run_stem}_theory.pt")
        torch.save(
            {
                "variant": variant,
                "task": task,
                ...
                "delta_theta": None if theory_artifact is None else theory_artifact["delta_theta"],
                "module_names": None if theory_artifact is None else theory_artifact["module_names"],
                "index_meta": None if theory_artifact is None else theory_artifact["index_meta"],
                "sparse_mask": None if theory_artifact is None else theory_artifact["sparse_mask"],
                "classifier_state": classifier_state,
                "eval_logits": eval_logits,
            },
            artifact_path,
        )
        print(f"Saved theory artifacts to {artifact_path}")
    ...
    with open(out_path, "w") as f:
        json.dump({
            ...
            "actual_train_size": len(datasets["train"]),
            "actual_subset_ratio": actual_subset_ratio,
            "n_flipped_labels": n_flipped_labels,
            "total_steps": total_steps,
