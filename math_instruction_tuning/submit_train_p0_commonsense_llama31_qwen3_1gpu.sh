#!/bin/bash
#SBATCH --job-name=p0_cs_train_5880
#SBATCH --nodes=1
...
#SBATCH --array=0-11
#SBATCH --output=logs/p0_cs_train_5880_%A_%a.out
#SBATCH --error=logs/p0_cs_train_5880_%A_%a.err

# P0 主实验(常识推理): Llama-3.1-8B + Qwen3-8B 在 Commonsense-170K (LLM-Adapters)
# 上联合训练, 之后用 submit_eval_p0_commonsense_*.sh 在 8 个基准上评测。
# 单卡 RTX5880(48GB): 8B 模型 + 梯度检查点 + bf16 单卡即可。
# 两阶段策略与数学脚本一致: TASK_ID = backbone_idx * 6 + config_idx
#   backbone: 0=llama31, 1=qwen3
#   config: 0=lora, 1=unilora, 2=prolosa 2:1, 3=prolosa 4:1, 4=prolosa 8:1, 5=prolosa 12:1
#   阶段1默认 SEED=42; 定最优比例后 SEED=43/44 补跑 lora/unilora/prolosa(best)。
# 训练数据需先在登录节点执行一次下载(见 data/commonsense/, 由 curl 从 LLM-Adapters 仓库获取)。
...
if [[ ! -f "${DATA_PATH}" ]]; then
    echo "Missing training data: ${DATA_PATH}. Download commonsense_170k.json first." >&2
    exit 1
fi
...
echo ">>> P0 commonsense training task_id=${TASK_ID}"
...
echo "P0 commonsense training finished: ${MODEL_TAG} ${METHOD} seed=${SEED}."
